            ...
            switch (item) {
                .FUNCTION => |method| {
                    ...
                    try self.visitFunctionStmt(
                        method.*,
                        method_field.kind.FUNC.args_size,
                        method_field.used,
                        method_field.name,
                    );
                },
        ...
        switch (mutStmt.op.kind) {
            ...
            .SLASH_EQUAL => {
                ...
                try self.print(
                    ...
                    \\    mov [{s}], {s}
                    \\
                , .{ sized_rax, size_keyword, id_reg.name, expr_reg.name, id_reg.name, sized_rax });
            },
            .PERCENT_EQUAL => {
                const sized_rax = switch (size) {
                    1 => "al",
                ...
                try self.print(
                    ...
                    \\    mov [{s}], {s}
                    \\
                , .{ sized_rax, size_keyword, id_reg.name, expr_reg.name, id_reg.name, sized_rdx });
            },
